...
namespace stdA {
    class room {
        public:
			...
			void setBetCrazyClub(bool _on);
			bool isBetCrazyClub() const { return m_ri.bet_crazy_club != 0u; };
			...
			bool isFull();
			bool isGaming();
			bool isGamingBefore(uint32_t _uid);
			bool isReconnecting(uint32_t _uid);

			uint32_t getReconnectOID(uint32_t _uid);
		...
		protected:
			...
			PlayerRoomInfoEx* makePlayerInvitedInfo(player& _session);
        ...
        protected:
			...
			std::map< uint32_t , bool > m_player_kicked;
