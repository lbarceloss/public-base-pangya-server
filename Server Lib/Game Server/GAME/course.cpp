...
Course::Course(RoomInfoEx& _ri, unsigned char _channel_rookie, float _star, uint32_t _rate_rain, unsigned char _rain_persist_flag)
	: m_ri(_ri), m_channel_rookie(_channel_rookie), m_star(_star), m_rate_rain(_rate_rain), m_rain_persist_flag(_rain_persist_flag), m_hole(), m_seq(),
		m_seed_rand_game(0u), m_flag_cube_coin(1u), m_wind_flag(0u), m_wind_range{ 0, 15 }, m_chr{ 0 }, m_holes_rain(0u), m_grand_prix_special_hole(false) {
...
void Course::makePacketHoleInfo(packet& _p, int _option, unsigned short _from_seq) {

	for (auto& el : m_hole) {

		if (el.first < _from_seq)
			continue;
	...
	makePacketHoleSpinningCubeInfo(_p, _from_seq);
}

void Course::makePacketHoleSpinningCubeInfo(packet& _p, unsigned short _from_seq) {
	...
	for (auto& el : m_hole) {
		...
		_p.addUint8((unsigned char)el.second.getCubes().size());
		...
		for (auto& el2 : el.second.getCubes()) {
			...
			_p.addUint8((unsigned char)(el.first - _from_seq));
		...
		if (!sh.empty()) {
			...
			if (m_seq.size() < 18) {
				...
				for (auto i = m_seq.size() + 1; i <= 18u; ++i)
					m_seq.push_back(Sequencia((unsigned char)m_ri.course , (unsigned short)i));
			}
...
void Course::init_hole() {
	...
	case ORCHID_BLOSSOM_ART:
		m_wind_range[1] = 8;
		break;
	case PENNE_ABACUS_ART:
		m_wind_flag = 1;
		break;
	case TITAN_WINDMILL_ART:
		m_wind_flag = 2;
		break;
	}
		...
		try {
			...
			if (gp != nullptr) {
				...
				case SUPER_WIND_RULE:
					...
					m_wind_range[1] = 15;
					break;
				case HOLE_CUP_MAGNET_RULE:
				case NO_TURNING_BACK_RULE:
					break;
