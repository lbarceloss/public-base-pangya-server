...
constexpr uint32_t PREMIUM_2_BALL_TYPEID = 0x140000E9u;

constexpr uint32_t PREMIUM_2_CLUBSET_TYPEID = 0x100000F7u;

constexpr uint32_t PREMIUM_2_AUTO_CALIPER_TYPEID = 0x1A000040u;

constexpr uint32_t PREMIUM_2_MASCOT_TYPEID = 0x4000004Bu;
...
void PremiumSystem::addPremiumUser(player& _session, WarehouseItemEx& _ticket, uint32_t _time) {
	...
	try {
		...
		_session.m_pi.pt.unix_sec_date = _ticket.end_date_unix_local - (uint32_t)GetLocalTimeAsUnix();
		...
		if (add_itens.size()) {
			...
			p.addUint32((uint32_t)add_itens.size());
