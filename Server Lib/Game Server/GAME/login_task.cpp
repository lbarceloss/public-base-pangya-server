...
void LoginTask::sendCompleteData() {
	...
	try {
		...
		packet_func::pacote169(p, &m_session, pi->ti_current_season, 5 );
		packet_func::session_send(p, &m_session, 0);
		packet_func::pacote169(p, &m_session, pi->ti_rest_season);
		packet_func::session_send(p, &m_session, 0);

		packet_func::pacote0B4(p, &m_session, pi->v_tsi_current_season, 5 );
		packet_func::session_send(p, &m_session, 0);
		...
		if (packet_func::pacote25D(p, &m_session, pi->v_tgp_current_season, 5 ))
			packet_func::session_send(p, &m_session, 0);
