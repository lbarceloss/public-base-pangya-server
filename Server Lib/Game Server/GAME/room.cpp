...
void room::enter(player& _session) {
	...
	try {
		...
		if (m_ri.master == -2 && isBetCrazyClub()) {

			m_ri.master = _session.m_pi.uid;

			_smp::message_pool::getInstance().push(new message("[CRAZY][room::enter][Log] sala[NUMERO="
				+ std::to_string((int)m_ri.numero) + "] da aposta ganhou master[UID="
				+ std::to_string(m_ri.master) + "] (primeiro a entrar).", CL_FILE_LOG_AND_CONSOLE));
		}
		...
		if (m_pGame == nullptr && v_sessions.size() > 0 && _session.m_pi.m_cap.stBit.game_master  && m_ri.state_flag != 0x100
...
int room::leave(player& _session, int _option) {
	...
	try {
		...
		if ((m_pGame == nullptr && m_ri.tipo == RoomInfo::TIPO::SPECIAL_SHUFFLE_COURSE && _session.m_pi.uid == m_ri.master)
			|| (_session.m_pi.m_cap.stBit.game_master  && m_ri.master == _session.m_pi.uid && m_ri.tipo != RoomInfo::TIPO::LOUNGE && m_ri.trofel == TROFEL_GM_EVENT_TYPEID)) {
	...
	return (v_sessions.size() > 0 || (m_ri.master == -2 && ( !isDropRoom() || m_ri.tipo >= RoomInfo::TIPO::GRAND_ZODIAC_INT && m_ri.tipo <= RoomInfo::TIPO::GRAND_ZODIAC_ADV))) ? 0 : 1;
};
...
void room::setNome(std::string _nome) {
	...
	{
		size_t _n = _nome.size() < sizeof(m_ri.nome) ? _nome.size() : sizeof(m_ri.nome) - 1;
		memset(m_ri.nome, 0, sizeof(m_ri.nome));
		memcpy(m_ri.nome, _nome.data(), _n);
	}
};

void room::setBetCrazyClub(bool _on) {

	unsigned int antes = m_ri.bet_crazy_club;

	m_ri.bet_crazy_club = _on ? 1u : 0u;

	if (m_ri.bet_crazy_club != antes)
		_smp::message_pool::getInstance().push(new message("[CRAZY][room::setBetCrazyClub][Log] sala[NUMERO="
			+ std::to_string((int)m_ri.numero) + ", MASTER=" + std::to_string(m_ri.master) + "] BET_CRAZY_CLUB "
			+ (m_ri.bet_crazy_club ? "LIGADO" : "DESLIGADO"), CL_FILE_LOG_AND_CONSOLE));
};
...
void room::setSenha(std::string _senha) {
	...
	if (_senha.empty()) {
		...
		{
			size_t _n = _senha.size() < sizeof(m_ri.senha) ? _senha.size() : sizeof(m_ri.senha) - 1;
			memset(m_ri.senha, 0, sizeof(m_ri.senha));
			memcpy(m_ri.senha, _senha.data(), _n);
		}
...
bool room::isReconnecting(uint32_t _uid) {

	if (_uid == 0u || m_pGame == nullptr)
		return false;

	return m_pGame->isReconnecting(_uid);
};

uint32_t room::getReconnectOID(uint32_t _uid) {

	if (_uid == 0u || m_pGame == nullptr)
		return 0u;

	return m_pGame->getReconnectOID(_uid);
};
...
#endif
	pri.position = (unsigned char)getPosition(&_session) + 1;
	...
	try {
		...
		for (auto i = 0u; i < num_info; ++i) {
			...
			case RoomInfo::INFO_CHANGE::NATURAL:
			{
				...
				if (sgs::gs::getInstance().getInfo().propriedade.stBit.natural)
		...
		packet_func::pacote04A(p, m_ri, 25 );
		packet_func::session_send(p, &_session, 1);
	}
				...
				for (auto& el : gm) {
					...
					if (((el->m_gi.channel && el->m_pi.channel == c->getInfo()->id) || el->m_gi.whisper || el->m_gi.isOpenPlayerWhisper(_session.m_pi.uid))
						&&  (el->m_pi.channel != _session.m_pi.channel || el->m_pi.mi.sala_numero != _session.m_pi.mi.sala_numero
							||  team.findPlayerByUID(el->m_pi.uid) == nullptr)) {
	...
	try {
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_CADDIE:
		{
			...
			if (_cpir.caddie != 0 && (pCi = _session.m_pi.findCaddieById(_cpir.caddie)) != nullptr
				...
				error = (_cpir.caddie == 0) ? 1  : (pCi == nullptr ? 2  : 3 );
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_BALL:
		{
			...
			if (_cpir.ball != 0 && (pWi = _session.m_pi.findWarehouseItemByTypeid(_cpir.ball)) != nullptr
				...
				_session.m_pi.ue.ball_typeid = _cpir.ball;
				...
				if (pWi != nullptr) {
					...
					if (item._typeid != 0) {
						...
						if ((_cpir.ball = item_manager::addItem(item, _session, 2 , 0)) != item_manager::RetAddItem::T_ERROR) {
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_CLUBSET:
		{
			...
			if (_cpir.clubset != 0 && (pWi = _session.m_pi.findWarehouseItemById(_cpir.clubset)) != nullptr
					...
					if (cs != nullptr) {
						...
						if (pWi != nullptr) {
							...
							if (item._typeid != 0) {
								...
								if ((_cpir.clubset = item_manager::addItem(item, _session, 2 , 0)) != item_manager::RetAddItem::T_ERROR) {
				...
				error = (_cpir.clubset == 0) ? 1  : (pWi == nullptr ? 2  : 3 );
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_CHARACTER:
		{
			...
			if (_cpir.character != 0 && (pCe = _session.m_pi.findCharacterById(_cpir.character)) != nullptr
				...
				error = (_cpir.character == 0) ? 1  : (pCe == nullptr ? 2  : 3 );
				...
				if (_session.m_pi.mp_ce.size() > 0) {
					...
					if (item._typeid != 0) {
						...
						if ((_cpir.character = item_manager::addItem(item, _session, 2 , 0)) != item_manager::RetAddItem::T_ERROR) {
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_MASCOT:
		{
			...
			if (_cpir.mascot != 0) {
				...
				if ((pMi = _session.m_pi.findMascotById(_cpir.mascot)) != nullptr && sIff::getInstance().getItemGroupIdentify(pMi->_typeid) == iff::MASCOT) {
					...
					error = (_cpir.mascot == 0) ? 1  : (pMi == nullptr ? 2  : 3 );
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_ITEM_EFFECT_LOUNGE:
		{
			...
			if (_session.m_pi.ei.char_info == nullptr)
				throw exception("[room::requestChangePlayerItemRoom][Error] Player[UID=" + std::to_string(_session.m_pi.uid)
						...
						STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 1000, 0x57007 ));
		...
		case ChangePlayerItemRoom::TYPE_CHANGE::TC_ALL:
		{
			...
			if (_session.m_pi.ue.mascot_id != 0 && _session.m_pi.ei.mascot_info != nullptr) {
				...
				if (m_it != _session.m_pi.mp_ui.end()) {
					...
					snmdb::NormalManagerDB::getInstance().add(0, new CmdUpdateMascotEquiped(_session.m_pi.uid, 0 ), room::SQLDBResponse, this);
...
void room::requestChangeTeam(player& _session, packet *_packet) {
	...
	try {
		...
		try {
			...
			m_teans[pPri->state_flag.uFlag.stFlagBit.team].deletePlayer(&_session, 3 );
		...
		pPri->state_flag.uFlag.stFlagBit.team = team;
...
constexpr uint32_t SOLO_TOURNEY_TICKET_TYPEID = 0x1A000137u;
...
bool room::requestStartGame(player& _session, packet *_packet) {
	...
	try {
		...
		const bool solo_tourney = (v_sessions.size() == 1
				&& (m_ri.tipo == RoomInfo::TIPO::TOURNEY || m_ri.tipo == RoomInfo::TIPO::SPECIAL_SHUFFLE_COURSE));

		if (!m_bot_tourney && !solo_tourney && v_sessions.size() == 1 && m_ri.tipo != RoomInfo::PRACTICE && m_ri.tipo != RoomInfo::GRAND_PRIX
		...
		if (solo_tourney) {
			...
			auto pTicket = _session.m_pi.findWarehouseItemByTypeid(SOLO_TOURNEY_TICKET_TYPEID);

			if (pTicket != nullptr && pTicket->STDA_C_ITEM_QNTD >= 1) {
				...
				if (item_manager::removeItem(item, _session) <= 0)
					throw exception("[room::requestStartGame][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + "] tentou comecar o Tourney sozinho na sala[NUMERO="
							+ std::to_string(m_ri.numero) + "], mas nao conseguiu deletar o Ticket[TYPEID=" + std::to_string(item._typeid)
							+ ", ID=" + std::to_string(item.id) + "]. Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::ROOM, 60, 500017));
			...
			_smp::message_pool::getInstance().push(new message("[room::requestStartGame][Log] Solo Tourney - Player[UID=" + std::to_string(_session.m_pi.uid)
					+ "] comecou a sala[NUMERO=" + std::to_string(m_ri.numero) + ", TIPO=" + std::to_string((unsigned short)m_ri.tipo)
					+ "] sozinho, sem Bot.", CL_FILE_LOG_AND_CONSOLE));
		}
...
void room::requestSendTimeGame(player& _session) {
	...
	try {
		...
		p.addUint8((unsigned char)((STDA_SOURCE_ERROR_DECODE(e.getCodeError()) == STDA_ERROR_TYPE::ROOM) ? STDA_SYSTEM_ERROR_DECODE(e.getCodeError()) : 1 ));
...
bool room::requestEnterGameAfterStarted(player& _session) {
	...
	try {
		...
		bool reconectando = m_pGame->isReconnecting(_session.m_pi.uid);

		if (isGamingBefore(_session.m_pi.uid) && !reconectando)
			throw exception("[room::requestEnterGameAfterStarted][WARNING] player[UID=" + std::to_string(_session.m_pi.uid) + "] tentou entrar na sala[NUMERO="
		...
		if (!reconectando) {
			...
			if (remain > 0)
				remain /= STDA_10_MICRO_PER_MILLI;
			...
			if (remain >= tempo)
				throw exception("[room::requestEnterGameAfrerStarted][WARNING] player[UID=" + std::to_string(_session.m_pi.uid) + "] tentou entrar na sala[NUMERO="
						+ std::to_string(m_ri.numero) + "] ja em jogo, mas o tempo de entrar no tourney acabou.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::ROOM, 2706, 2));
		}

		if (reconectando)
			m_pGame->rebindSession(_session.m_pi.uid, _session);
...
void room::requestExecCCGGoldenBell(player& _session, packet *_packet) {
	...
	try {
		...
		for (auto& el : v_sessions) {
			...
			item_manager::initItemFromBuyItem(el->m_pi, item, bi, false, 0, 0, 1 );
...
bool room::execGhostCalcCmd(player& _session, std::string& _msg) {
	...
	try {
		...
		if (!isGaming())
			throw exception("[room::execGhostCalcCmd][Error] Player[UID=" + std::to_string(_session.m_pi.uid)
					+ "] Channel[ID=" + std::to_string((unsigned short)m_channel_owner) + "] tentou executar Ghost Calc Command na sala[NUMERO="
					+ std::to_string(m_ri.numero) + "], mas ele nao esta em jogo.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::ROOM, 10000, 0));

		ret = m_pGame->execGhostCalcCmd(_session, _msg);

	}catch (exception& e) {

		_smp::message_pool::getInstance().push(new message("[room::execGhostCalcCmd][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
		...
		if (m_ri.flag == 0) {
			...
			try {
				...
				if (m_ri.tipo == RoomInfo::TIPO::STROKE || m_ri.tipo == RoomInfo::TIPO::MATCH) {
					...
					if (packet_func::pacote048(p48, &_session, v_element, 0))
						packet_func::session_send(p48, &_session, 1);

					_smp::message_pool::getInstance().push(new message("[room::startGame][Log] LATE-JOIN: mandei a lista completa de "
							+ std::to_string(v_element.size()) + " players via 0x48 opt 0 (UNICAST) para o player[UID="
							+ std::to_string(_session.m_pi.uid) + "] montar o mapa OID->indice do golfe.", CL_FILE_LOG_AND_CONSOLE));

					int nivel_sync48 = 1;
					{
						FILE* f48 = nullptr;

						if (fopen_s(&f48, "late_join_sync48.txt", "r") == 0 && f48 != nullptr) {

							if (fscanf_s(f48, "%d", &nivel_sync48) != 1)
								nivel_sync48 = 1;
					...
					if (nivel_sync48 != 0) {
						...
						for (auto& el : v_sessions) {
							...
							if (packet_func::pacote048(p48_el, el, v_element, 0))
								packet_func::session_send(p48_el, el, 1);
						}

						_smp::message_pool::getInstance().push(new message("[room::startGame][Log] LATE-JOIN: mandei a MESMA lista completa de "
								+ std::to_string(v_element.size()) + " players via 0x48 opt 0 (UNICAST) para os outros "
								+ std::to_string(v_sessions.size() - 1) + " players ja em jogo registrarem o late-joiner[UID="
								+ std::to_string(_session.m_pi.uid) + "] no mapa OID->indice do golfe deles.", CL_FILE_LOG_AND_CONSOLE));
					}else
						_smp::message_pool::getInstance().push(new message("[room::startGame][Log] LATE-JOIN: late_join_sync48.txt = 0, NAO mandei a lista para os players ja em jogo (bissecao).", CL_FILE_LOG_AND_CONSOLE));
				}
...
void room::updateMaster(player* _session) {
	...
	if (_session != nullptr && _session->m_pi.m_cap.stBit.game_master  && m_ri.master != -2) {

		if (master == nullptr || !(master->m_pi.m_cap.stBit.game_master )) {
	...
	}else if (master == nullptr && v_sessions.size() > 0 && m_ri.master != -2) {
...
bool room::isAllReady() {
	...
	if (m_ri.tipo == RoomInfo::TIPO::PRACTICE || m_ri.tipo == RoomInfo::TIPO::GRAND_ZODIAC_PRACTICE
			|| (m_ri.tipo == RoomInfo::TIPO::GRAND_PRIX  ))
		return true;
	...
	auto count = std::count_if(v_sessions.begin(), v_sessions.end(), [&](auto& _el) {
		...
		return (pri != nullptr && pri->state_flag.uFlag.stFlagBit.ready);
	});
	...
	if (m_pGame != nullptr) {
		...
		for (auto& el : m_player_info) {
			...
			if (m_ri.master == el.first->m_pi.uid)
				...
				el.second.state_flag.uFlag.stFlagBit.ready = 0 ;
		...
		if (m_ri.course >= 0x7Fu)
			m_ri.course = RoomInfo::eCOURSE(0x7Fu);
...
void room::init_teans() {
	...
	m_teans.push_back(Team(0 ));
	m_teans.push_back(Team(1 ));
...
void room::addBotVisual(player& _session) {
	...
	try {
		...
		pri.char_typeid = 0x4000000u;
		pri.title = 0x39800013u;
#if defined(_WIN32)
