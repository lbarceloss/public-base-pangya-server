...
Approach::Approach(std::vector< player* >& _players, RoomInfoEx& _ri, RateValue _rv, unsigned char _channel_rookie)
	...
	m_thread_sync_hole = new thread(1060 , Approach::_syncHoleTime, (LPVOID)this);
...
void Approach::requestTranslateSyncShotData(player& _session, ShotSyncData& _ssd) {
	...
	try {
		...
		if (_session.m_pi.uid == s->m_pi.uid) {
			...
			if (_ssd.state == ShotSyncData::SHOT_STATE::INTO_HOLE || std::abs(hole->getPinLocation().diffXZ(pgi->location) * MEDIDA_PARA_YARDS) <= 0.08 )
	...
	if (m_mission.numero > 0) {
		...
		case 6:
		{
			...
			if (it != m_player_info.end()) {
				...
				case 1:
					win = it->second->mascot_typeid == 0x40000002u;
					break;
				case 2:
					win = it->second->mascot_typeid == 0x40000001u;
					break;
				case 3:
					win = it->second->mascot_typeid == 0x40000003u;
					break;
				case 4:
					win = it->second->mascot_typeid == 0x40000000u;
					break;
				}
			}
		...
		case 10:
		{
			...
			std::for_each(m_player_info.begin(), m_player_info.end(), [&](auto& _el) {
				...
				if (_el.second != nullptr && _el.second->flag != PlayerGameInfo::eFLAG_GAME::QUIT) {
					...
					if (pai->m_app_dados.state.ucState == 0u && pai->m_app_dados.distance < 100
		...
		case 23:
		{
			std::for_each(m_player_info.begin(), m_player_info.end(), [&](auto& _el) {
				...
				if (_el.second != nullptr && _el.second->flag != PlayerGameInfo::eFLAG_GAME::QUIT) {
					...
					if (pai->m_app_dados.state.ucState == 0u && pai->m_app_dados.distance < 200  && _el.first->m_pi.ei.cad_info != nullptr) {
