...
void TourneyBase::requestSyncShot(player& _session, packet *_packet) {
	...
	try {
		...
		requestCalculeShotSpinningCube(_session, ssd);

		requestCalculeShotCoin(_session, ssd);
	...
	try {
		...
		if (item_typeid == MULLIGAN_ROSE_TYPEID) {
			...
			if (std::count_if(_session.m_pi.v_cei.begin(), _session.m_pi.v_cei.end(), [](auto& _el) {
				return (_el.parts_id == 0 && _el.parts_typeid == 0 && sIff::getInstance().getItemSubGroupIdentify22(_el._typeid) == 2  && _el.efeito == 32 );
			}) > 0) {
		...
		if (WIND_ITEM_TYPEID != 0u && pWi->_typeid == WIND_ITEM_TYPEID) {
			...
			_smp::message_pool::getInstance().push(new message(
				"[TourneyBase::requestUseActiveItem][WindItem] login='" + login + "' armWind="
...
void TourneyBase::requestSendTimeGame(player& _session) {
	...
	try {
		...
		p.addUint8((unsigned char)((STDA_SOURCE_ERROR_DECODE(e.getCodeError()) == STDA_ERROR_TYPE::TOURNEY_BASE) ? STDA_SYSTEM_ERROR_DECODE(e.getCodeError()) : 1 ));
...
void TourneyBase::startTime() {
	...
	job j(TourneyBase::end_time, this, nullptr);
		...
		m_timer = sgs::gs::getInstance().makeTime(m_ri.time_30s , j);
...
void TourneyBase::calcule_shot_to_coin(player& _session, ShotSyncData& _ssd) {
	...
	try {
		...
		constexpr float MIN_DISTANCE_TO_HOLE_TO_SPAWN_COIN = 70.f * SCALE_PANGYA;
