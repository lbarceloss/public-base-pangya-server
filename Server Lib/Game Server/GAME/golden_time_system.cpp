...
void GoldenTimeSystem::initialize() {
	...
	CmdGoldenTimeInfo cmd_gti(true);
	...
	CmdGoldenTimeItem cmd_gt_item(true);
	CmdGoldenTimeRound cmd_gt_round(true);
...
void GoldenTimeSystem::updateGoldenTimeEnd() {
	...
	if (m_current_golden_time->type != stGoldenTime::eTYPE::ONE_DAY) {
		...
		m_current_golden_time->date[0] = UnixToSystemTime(SystemTimeToUnix(m_current_golden_time->date[0]) + (24 * 60 * 60));
...
stGoldenTimeReward GoldenTimeSystem::calculeRoundReward(std::vector< stPlayerReward > _player_reward) {
	...
	while (number_of_winners > 0) {
		...
		if ((ctx = lottery.spinRoleta(true )) != nullptr && (uint32_t*)ctx->value != nullptr) {
