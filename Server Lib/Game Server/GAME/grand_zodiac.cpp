...
GrandZodiac::GrandZodiac(std::vector< player* >& _players, RoomInfoEx& _ri, RateValue _rv, unsigned char _channel_rookie)
	...
	for (auto& el : m_players) {
		...
		pgi->sys_achieve.incrementCounter(0x6C40003Cu );
	}
...
void GrandZodiac::requestMakeTrofel() {
	...
	if (players_num >= 20u && players_num < 30u)
		qntd = 1u;
	else if (players_num >= 30 && players_num < 50)
		qntd = 2u;
	else if (players_num >= 50)
		qntd = 3u;
...
void GrandZodiac::endGoldenBeam() {
	...
	try {
		...
		if (!m_mp_golden_beam_player.empty()) {
			...
			uint64_t jackpot = m_players.size() * (sRandomGen::getInstance().rIbeMt19937_64_chronoRange(1, 5) * 500);

			auto seed = sRandomGen::getInstance().rIbeMt19937_64_chronoRange(0, 1);

			if (m_mp_golden_beam_player.size() == 1 || seed == 0ull ) {
	...
	&& _session.getState() && _session.isConnected() && m_players.size() > 0) {
		...
		if (option == 0x12C  || option == 2 ) {
			...
			if (m_grand_zodiac_state)
				finish_grand_zodiac(_session, (option == 0x12C && !is_hacker_or_bug) ? 0 : 1);
		}
	}
