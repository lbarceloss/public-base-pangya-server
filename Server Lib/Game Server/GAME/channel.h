...
namespace stdA {
    class channel {
        ...
        public:
			...
			void leaveChannel(player& _session, int _option = 0);
			...
			void leaveLobby(player& _session, int _option = 0);
			...
			bool execGhostCalcCmd(player& _session, std::string& _msg);

			bool execBetCmd(player& _session, std::string& _msg);

			short criarSalaAposta(uint32_t _criador_uid, unsigned char _course,
					unsigned char _holes, const char* _pin);

			bool salaApostaViva(short _numero);
		...
		public:
			...
			void requestCheckInvite(player& _session, packet *_packet);
			...
			void requestPlayScratchCard(player& _session, packet* _packet);
			...
			void requestPlayBigPapelShop(player& _session, packet *_packet);
        ...
        private:
			...
			CRITICAL_SECTION m_cs_invite;
#elif defined(__linux__)
			pthread_mutex_t m_cs;
			pthread_mutex_t m_cs_invite;
#endif
    };
}
