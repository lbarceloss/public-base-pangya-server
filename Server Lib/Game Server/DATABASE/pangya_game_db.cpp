...
UserInfo pangya_db::getUserInfo(int32_t uid) {
	...
	for (i = 0; i < 5; i++)
		ui.best_score[i] = (unsigned char)IFNULL(atoi, _result->data[24 + i]);
	for (i = 0; i < 5; i++)
		ui.best_pang[i] = IFNULL(atoll, _result->data[29 + i]);
...
TrofelInfo pangya_db::getTrofelInfo(int32_t uid, int season) {
	...
	for (i = 0; i < 3; i++)
		ti.ama_6[i] = (short)IFNULL(atoi, _result->data[0 + i]);
	for (i = 0; i < 3; i++)
		ti.ama_5[i] = (short)IFNULL(atoi, _result->data[3 + i]);
	for (i = 0; i < 3; i++)
		ti.ama_4[i] = (short)IFNULL(atoi, _result->data[6 + i]);
	for (i = 0; i < 3; i++)
		ti.ama_3[i] = (short)IFNULL(atoi, _result->data[9 + i]);
	for (i = 0; i < 3; i++)
		ti.ama_2[i] = (short)IFNULL(atoi, _result->data[12 + i]);
	for (i = 0; i < 3; i++)
		ti.ama_1[i] = (short)IFNULL(atoi, _result->data[15 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_1[i] = (short)IFNULL(atoi, _result->data[18 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_2[i] = (short)IFNULL(atoi, _result->data[21 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_3[i] = (short)IFNULL(atoi, _result->data[24 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_4[i] = (short)IFNULL(atoi, _result->data[27 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_5[i] = (short)IFNULL(atoi, _result->data[30 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_6[i] = (short)IFNULL(atoi, _result->data[33 + i]);
	for (i = 0; i < 3; i++)
		ti.pro_7[i] = (short)IFNULL(atoi, _result->data[36 + i]);
...
UserEquip pangya_db::getUserEquip(int32_t uid) {
	...
	for (i = 0; i < 10; i++)
		ue.item_slot[i] = IFNULL(atoi, _result->data[4 + i]);
	for (i = 0; i < 6; i++)
		ue.skin_id[i] = IFNULL(atoi, _result->data[14 + i]);
	for (i = 0; i < 6; i++)
		ue.skin_typeid[i] = IFNULL(atoi, _result->data[20 + i]);
	ue.mascot_id = IFNULL(atoi, _result->data[26]);
	for (i = 0; i < 2; i++)
		ue.poster[i] = IFNULL(atoi, _result->data[27 + i]);
...
ClubSetInfo pangya_db::getClubSetInfo(int32_t uid, int32_t clubset_id) {
	...
	for (i = 0; i < 5; i++)
		csi.enchant_c[i] = (short)IFNULL(atoi, _result->data[2 + i]);
	for (i = 0; i < 5; i++)
		csi.slot_c[i] = (short)IFNULL(atoi, _result->data[7 + i]);
...
std::vector< WarehouseItem > pangya_db::getWarehouseItem(int32_t uid) {
	...
	for (i = 0; i < 5; i++)
		wi.c[i] = (short)IFNULL(atoi, _result->data[4 + i]);
	...
	for (i = 0; i < 4; i++)
		wi.card.character[i] = IFNULL(atoi, _result->data[15 + i]);
	for (i = 0; i < 4; i++)
		wi.card.caddie[i] = IFNULL(atoi, _result->data[19 + i]);
	for (i = 0; i < 4; i++)
		wi.card.NPC[i] = IFNULL(atoi, _result->data[23 + i]);
	wi.clubset_workshop.flag = (short)IFNULL(atoi, _result->data[27]);
	for (i = 0; i < 5; i++)
		wi.clubset_workshop.c[i] = (short)IFNULL(atoi, _result->data[28 + i]);
...
std::map< size_t, AchievementInfo > pangya_db::getAchievementInfo(int32_t uid) {
	...
	qsi._typeid = IFNULL(atoi, _result->data[4]);
...
std::vector< CardEquipInfo > pangya_db::getCardEquipInfo(int32_t uid) {
	exec_query query("pangya.ProcGetCardEquip", std::to_string(uid) + ", 0");
...
std::vector< YamUsed > pangya_db::getYamUsed(int32_t uid) {
	exec_query query("pangya.ProcGetCardEquip", std::to_string(uid) + ", 1");
...
EmailInfo pangya_db::getEmailInfo(int32_t uid, int32_t email_id) {
	...
	if (item._typeid != 0) {
		...
		item.id = IFNULL(atoi, _result->data[5]);
...
Last5PlayersGame pangya_db::getLast5PlayersGame(int32_t uid) {
	...
	for (i = 0; i < 5; i++) {
		...
		if (_result->data[i * 4 + 1] != nullptr)
			strcpy_s(l5pg.players[i].nick, _result->data[i * 4 + 1]);
