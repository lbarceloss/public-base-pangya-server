...
namespace stdA {
...
#endif
	{
		public:
			...
			bool reclaimOID(uint32_t _oid_cadeira, uint32_t _oid_novo);
		...
		protected:
			...
			virtual bool checkGhostCalcCmd(player& _session, std::string& _msg);

			virtual bool checkBetCmd(player& _session, std::string& _msg);

			static int betLobbyTick(void* _arg1, void* _arg2);
