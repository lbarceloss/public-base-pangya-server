...
std::multimap< int32_t , WarehouseItemEx >::iterator PlayerInfo::findWarehouseItemItById(int32_t _id) {

	return mp_wi.find(_id);
}

std::multimap< int32_t , WarehouseItemEx >::iterator PlayerInfo::findWarehouseItemItByTypeid(uint32_t _typeid) {
...
std::multimap< int32_t , WarehouseItemEx >::iterator PlayerInfo::findWarehouseItemItByTypeidAndId(uint32_t _typeid, int32_t _id) {
...
std::multimap< int32_t , CharacterInfo >::iterator PlayerInfo::findCharacterItById(int32_t _id) {

	return mp_ce.find(_id);
}

std::multimap< int32_t , CharacterInfo >::iterator PlayerInfo::findCharacterItByTypeid(uint32_t _typeid) {
...
std::multimap< int32_t , CharacterInfo >::iterator PlayerInfo::findCharacterItByTypeidAndId(uint32_t _typeid, int32_t _id) {
...
std::multimap< int32_t , CaddieInfoEx >::iterator PlayerInfo::findCaddieItById(int32_t _id) {

	return mp_ci.find(_id);
}

std::multimap< int32_t , CaddieInfoEx >::iterator PlayerInfo::findCaddieItByTypeid(uint32_t _typeid) {
...
std::multimap< int32_t , CaddieInfoEx >::iterator PlayerInfo::findCaddieItByTypeidAndId(uint32_t _typeid, int32_t _id) {
...
std::multimap< int32_t , MascotInfoEx >::iterator PlayerInfo::findMascotItById(int32_t _id) {

	return mp_mi.find(_id);
}

std::multimap< int32_t , MascotInfoEx >::iterator PlayerInfo::findMascotItByTypeid(uint32_t _typeid) {
...
std::multimap< int32_t , MascotInfoEx >::iterator PlayerInfo::findMascotItByTypeidAndId(uint32_t _typeid, int32_t _id) {
...
std::vector< WarehouseItemEx* > PlayerInfo::findAllPartNotEquiped(uint32_t _typeid) {
	...
	std::for_each(mp_wi.begin(), mp_wi.end(), [&](auto& el) {
		...
		if (el.second._typeid == _typeid && (el.second.flag & 96) != 96  && (el.second.flag & 0x20) != 0x20 && (el.second.flag & 0x40) != 0x40 && (it = std::find_if(mp_ce.begin(), mp_ce.end(), [&](auto& el2) {
...
std::multimap< PlayerInfo::stIdentifyKey , UpdateItem >::iterator PlayerInfo::findUpdateItemByTypeid(uint32_t _typeid) {
...
std::multimap< PlayerInfo::stIdentifyKey , UpdateItem >::iterator PlayerInfo::findUpdateItemByType(UpdateItem::UI_TYPE _type) {
...
std::vector< std::multimap< PlayerInfo::stIdentifyKey , UpdateItem >::iterator > PlayerInfo::findUpdateItemByTypeidAndId(uint32_t _typeid, int32_t _id) {

	std::vector< std::multimap< PlayerInfo::stIdentifyKey , UpdateItem >::iterator > v_it;
...
std::multimap< PlayerInfo::stIdentifyKey , UpdateItem >::iterator PlayerInfo::findUpdateItemByIdAndType(int32_t _id, UpdateItem::UI_TYPE _type) {
...
std::multimap< PlayerInfo::stIdentifyKey , UpdateItem >::iterator PlayerInfo::findUpdateItemByTypeidAndType(uint32_t _typeid, UpdateItem::UI_TYPE _type) {
...
ExtraPower PlayerInfo::getExtraPower(bool _pwr_condition_actived) {
	...
	if (ei.mascot_info != nullptr) {
		...
		if (mascot != nullptr)
			power_drive.m_mascot += mascot->efeito.power_drive;
	}
		...
		if (character != nullptr) {
						...
						if (ab != nullptr) {
							...
							if ((ab->efeito.type[0] == 11 && ab->efeito.rate[0] == 0)
								...
								if (_pwr_condition_actived)
									power_drive.m_auxpart += auxpart->efeito.power_drive;

							}else {

								for (auto j = 0u; j < (sizeof(ab->efeito.type) / sizeof(ab->efeito.type[0])); ++j) {

									if (ab->efeito.type[j] == 13 ) {
			...
			for (auto i = 0u; i < (sizeof(ei.char_info->card_character) / sizeof(ei.char_info->card_character[0])); ++i) {
				...
				if (ei.char_info->card_character[i] != 0) {
					...
					if (card != nullptr && sIff::getInstance().getItemSubGroupIdentify22(card->_typeid) == 0
							&& card->efeito.type == 1 )
			...
			for (auto i = 0u; i < (sizeof(ei.char_info->card_caddie) / sizeof(ei.char_info->card_caddie[0])); ++i) {
				...
				if (ei.char_info->card_caddie[i] != 0) {
					...
					if (card != nullptr && sIff::getInstance().getItemSubGroupIdentify22(card->_typeid) == 1 ) {

						if (card->efeito.type == 5 )
							power_shot.m_card += card->efeito.qntd;
						else if (card->efeito.type == 2 )
							power_drive.m_card += card->efeito.qntd;
					}
				}
			}
	...
	for (auto it = v_cei.begin(); it != v_cei.end(); ++it) {
		...
		if (it->parts_id == 0 && it->parts_typeid == 0 && sIff::getInstance().getItemSubGroupIdentify22(it->_typeid) == 2
				&& it->efeito == 27)
...
uint32_t PlayerInfo::getSlotPower() {
	...
	for (auto it = v_cei.begin(); it != v_cei.end(); ++it) {
		...
		if (it->parts_id == 0 && it->parts_typeid == 0 && sIff::getInstance().getItemSubGroupIdentify22(it->_typeid) == 2  && it->efeito == 5)
	...
	if (set != nullptr) {
		for (auto i = 0u; i < (sizeof(set->packege.item_typeid) / sizeof(set->packege.item_typeid[0])); ++i) {
			...
			if (set->packege.item_typeid[i] != 0 && sIff::getInstance().getItemGroupIdentify(set->packege.item_typeid[i]) != iff::CHARACTER)
				if (ownerItem(set->packege.item_typeid[i]))
					return true;
		}
	}
...
bool PlayerInfo::ownerMailBoxItem(unsigned _typeid) {
	...
	CmdFindMailBoxItem cmd_fmbi(uid, _typeid, true);
...
void PlayerInfo::updateTrofelInfo(uint32_t _trofel_typeid, unsigned char _trofel_rank) {
	...
	if (_trofel_typeid == TROFEL_GM_EVENT_TYPEID )
		throw exception("[PlayerInfo::updateTrofelInfo][Error] player[UID=" + std::to_string(uid) + "] tentou atualizar um trofel[TYPEID="
	...
	if (sIff::getInstance().getItemSubGroupIdentify24(_trofel_typeid) != 0 && type > 12 )
		throw exception("[PlayerInfo::updateTrofelInfo][Error] player[UID=" + std::to_string(uid) + "] tentou atualizar um trofel[TYPEID="
...
void PlayerInfo::updateTrofelInfo(uint32_t _uid, uint32_t _trofel_typeid, unsigned char _trofel_rank) {
	...
	CmdTrofelInfo cmd_ti(_uid, CmdTrofelInfo::CURRENT, true);
