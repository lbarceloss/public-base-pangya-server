...
static uint32_t getPktLogUID() {

	static uint32_t s_uid = 0u;
	static clock_t s_last = 0;

	auto now = std::clock();

	if (s_last == 0 || (now - s_last) > CLOCKS_PER_SEC) {
		...
		if (fopen_s(&f, "pktlog_uid.txt", "r") == 0 && f != nullptr) {

			if (fscanf_s(f, "%u", &uid) != 1)
				uid = 0u;
...
static void pkt_record(stdA::packet& _p, stdA::session* _s) {

	if (_s == nullptr)
		return;

	auto uid = ((stdA::player*)_s)->m_pi.uid;

	auto buf = _p.getPlainBuf();

	if (buf.buf == nullptr || buf.len == 0)
		return;

	pkttap::record(pkttap::SRC_GAME, pkttap::DIR_OUT, uid, _s->m_oid, _p.getTipo(), buf.buf,
			(uint32_t)buf.len);

	auto alvo = getPktLogUID();
	...
	std::string nome = "pktlog_" + std::to_string(uid) + ".txt";

	if (fopen_s(&f, nome.c_str(), "a") != 0 || f == nullptr)
		return;
	...
	fprintf(f, "[%02u:%02u:%02u.%03u] UID=%u OID=%u op=0x%04X len=%u  ",
			st.wHour, st.wMinute, st.wSecond, st.wMilliseconds,
			uid, _s->m_oid, _p.getTipo(), buf.len);

	for (auto i = 0u; i < buf.len; ++i)
		fprintf(f, "%02X ", (unsigned char)buf.buf[i]);
...
int packet_func::packet007(void* _arg1, void* _arg2) {
	...
	try {
		...
		if (nc == SUCCESS) {
			...
			if (cmd_vn.getLastCheck()) {
				...
				CmdMemberInfo cmd_mi(cmd_vn.getUID(), true);
	...
	try {
		...
		if (error == 0 && nc == NICK_IN_USE) {
			...
			p.addBuffer(&mi, sizeof(MemberInfo));
		}
...
int packet_func::packet038(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_SERVER(_arg1, _arg2);
	...
	try {
		...
		CHECK_SESSION_IS_AUTHORIZED("packet038");

		std::string nick_mb = pd._packet->readString();
		nick = MbToWc(nick_mb);

		_smp::message_pool::getInstance().push(new message(L"[packet_func::packet038][DIAG] UID="
				+ std::to_wstring(pd._session.m_pi.uid) + L" nick=[" + nick + L"]",
				CL_FILE_LOG_AND_CONSOLE));

		bool valido = true;

		if (std::regex_match(nick.begin(), nick.end(), std::wregex(L".*[ ].*"))
				|| nick.size() < 4
				|| std::regex_match(nick.begin(), nick.end(), std::wregex(L".*[\\^$&,\\?`´~\\|\"@#¨'%*!\\\\].*"))) {
			valido = false;
			_smp::message_pool::getInstance().push(new message(L"[packet_func::packet038][Log] nick invalido (tamanho/caracteres/espaco): "
					+ nick, CL_FILE_LOG_AND_CONSOLE));
		}
		...
		if (valido) {
			...
			if (cmd_vn.getLastCheck() && cmd_vn.getUID() != pd._session.m_pi.uid) {
				valido = false;
				_smp::message_pool::getInstance().push(new message(L"[packet_func::packet038][Log] nick ja em uso: "
						+ nick, CL_FILE_LOG_AND_CONSOLE));
			}
		}

		if (valido) {
			CmdSaveNick cmd_sn(pd._session.m_pi.uid, nick, true);
			...
			std::string nick_mb = WcToMb(nick);
			strcpy_s(pd._session.m_pi.nickname, nick_mb.c_str());

			_smp::message_pool::getInstance().push(new message(L"[packet_func::packet038][Log] nick SALVO com sucesso para: "
					+ nick + L" (UID=" + std::to_wstring(pd._session.m_pi.uid) + L"). Aplica ao relogar.", CL_FILE_LOG_AND_CONSOLE));
		}

	}catch (exception& e) {

		_smp::message_pool::getInstance().push(new message("[packet_func::packet038][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));

	}catch (std::exception& e) {

		_smp::message_pool::getInstance().push(new message(std::string("[packet_func::packet038][ErrorSystem] ") + e.what(), CL_FILE_LOG_AND_CONSOLE));
	}
...
int packet_func::packet0BB9(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_SERVER(_arg1, _arg2);

	try {

		uint32_t tick_ms = pd._packet->readUint32();
		float    factor = 0.0f, raw_max = 0.0f, amp_p = 0.0f;
		uint32_t b;
		b = pd._packet->readUint32(); memcpy(&factor,  &b, 4);
		b = pd._packet->readUint32(); memcpy(&raw_max, &b, 4);
		b = pd._packet->readUint32(); memcpy(&amp_p,   &b, 4);

		auto& ac = pd._session.m_ac;

		if (!ac.crazy_active) {
			_smp::message_pool::getInstance().push(new message("[CRAZY][packet0BB9][Suspeito] player[UID="
				+ std::to_string(pd._session.m_pi.uid) + "] reportou tacada com o modo DESLIGADO", CL_FILE_LOG_AND_CONSOLE));
			return 0;
		}

		const uint64_t TOLERANCIA_MS = 3000;
		uint64_t decorrido = GetTickCount64() - ac.crazy_hole_started_at;

		if ((uint64_t)tick_ms > decorrido + TOLERANCIA_MS) {
			_smp::message_pool::getInstance().push(new message("[CRAZY][packet0BB9][FRAUDE] player[UID="
				+ std::to_string(pd._session.m_pi.uid) + "] tickMs=" + std::to_string(tick_ms)
				+ " > decorrido=" + std::to_string(decorrido) + " (+tolerancia)", CL_FILE_LOG_AND_CONSOLE));
			return 0;
		}

		if (raw_max < 50.0f || raw_max > 400.0f) {
			_smp::message_pool::getInstance().push(new message("[CRAZY][packet0BB9][FRAUDE] player[UID="
				+ std::to_string(pd._session.m_pi.uid) + "] rawMax=" + std::to_string(raw_max)
				+ " fora da faixa plausivel (50..400)", CL_FILE_LOG_AND_CONSOLE));
			return 0;
		}

		if (amp_p < ac.crazy_amp - 0.01f || amp_p > ac.crazy_amp + 0.01f) {
			_smp::message_pool::getInstance().push(new message("[CRAZY][packet0BB9][FRAUDE] player[UID="
				+ std::to_string(pd._session.m_pi.uid) + "] ampP=" + std::to_string(amp_p)
				+ " != mandado pelo server=" + std::to_string(ac.crazy_amp), CL_FILE_LOG_AND_CONSOLE));
			return 0;
		}

		if (raw_max > 1.0f && amp_p > 0.0f) {
			float esperado = crazyExpectedFactor(ac.crazy_seed, tick_ms, raw_max, amp_p);
			float erro = factor - esperado;
			if (erro < 0.0f) erro = -erro;

			if (erro > 0.01f) {
				_smp::message_pool::getInstance().push(new message("[CRAZY][packet0BB9][FRAUDE] player[UID="
					+ std::to_string(pd._session.m_pi.uid) + "] factor=" + std::to_string(factor)
					+ " esperado=" + std::to_string(esperado) + " (seed=" + std::to_string(ac.crazy_seed)
					+ ", tickMs=" + std::to_string(tick_ms) + ", rawMax=" + std::to_string(raw_max) + ")", CL_FILE_LOG_AND_CONSOLE));
				return 0;
			}
		}

		_smp::message_pool::getInstance().push(new message("[CRAZY][packet0BB9][Log] player[UID="
			+ std::to_string(pd._session.m_pi.uid) + "] tickMs=" + std::to_string(tick_ms)
			+ " factor=" + std::to_string(factor) + " OK", CL_FILE_LOG_AND_CONSOLE));

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::packet0BB9][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
...
int packet_func::packet070(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_SERVER(_arg1, _arg2);
	...
	try {
		...
		if (c != nullptr)
			c->requestPlayScratchCard(pd._session, pd._packet);

	}catch (exception& e) {

		_smp::message_pool::getInstance().push(new message("[packet_func::packet070][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
...
int packet_func::packet08B(void* _arg1, void* _arg2) {
	...
	try {
		...
		CmdServerList cmd_sl(CmdServerList::MSN, true);
...
int packet_func::packet0B5(void* _arg1, void* _arg2) {
	...
	try {
		...
		if (from_uid == to_uid && pd._session.m_pi.mrc.allow_enter) {
			p.addUint32(1);
...
int packet_func::packet0B7(void* _arg1, void* _arg2) {
	...
	try {
		...
		packet p((unsigned short)0x168);

		p.addBuffer(&pri, sizeof(PlayerRoomInfoEx));

		session_send(p, &pd._session, 0);

		p.init_plain((unsigned short)0x12D);
...
int packet_func::packet0C1(void* _arg1, void* _arg2) {
	...
	try {
		...
		pd._session.m_pi.place = pd._packet->readUint8();
...
#else
		if (((std::clock() - pd._session.m_tick_bot) / (double)CLOCKS_PER_SEC) >= ((gs->getBotTTL() + (gs->getBotTTL() / 4) ) / 1000.0))
...
int packet_func::packet157(void* _arg1, void* _arg2) {
	...
	try {
		...
		if (pd._session.m_pi.uid == uid)
			mgr_achievement = &pd._session.m_pi.mgr_achievement;
		else if ((s = gs->findPlayer(uid)) != nullptr)
...
int packet_func::packet172(void* _arg1, void* _arg2) {
	...
	try {
		...
		packet p;
		p.init_plain((unsigned short)0x24D);
		p.addUint32(0);
		p.addUint32(0);
		p.addUint32(0);
		p.addUint32(1000000);
...
int packet_func::packet_sv4D(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_SERVER_RET(_arg1, _arg2);
...
int packet_func::pacote047(packet& p, std::vector< RoomInfo > v_element, int option) {
	...
	p.addUint8((unsigned char)((option == 0 ) ? v_element.size() : 1));
	p.addInt8((unsigned char)option);
	p.addInt16(-1);
...
int packet_func::pacote04B(packet& p, player *_session, unsigned char _type, int error, int _valor) {
	...
	if (error == 0) {
		...
		case 6:
		{
			...
			if (_valor == ChangePlayerItemRoom::stItemEffectLounge::TYPE_EFFECT::TE_TWILIGHT )
				p.addInt32(1);
			else {
				...
				if (_session->m_pi.ei.char_info != nullptr) {
					...
					switch (_valor) {
					case ChangePlayerItemRoom::stItemEffectLounge::TYPE_EFFECT::TE_BIG_HEAD:
						p.addFloat(it->second.scale_head);
						break;
					case ChangePlayerItemRoom::stItemEffectLounge::TYPE_EFFECT::TE_FAST_WALK:
	...
	if (err_code == 4) {
		...
		case 3:
			if (_session->m_pi.ei.comet != nullptr)
				...
				p.addZeroByte(sizeof(uint32_t));
			p.addUint32(_session->m_pi.ei.csi.id);
			break;
		case 4:
...
int packet_func::pacote070(packet& p, player *_session, std::multimap< int32_t , CharacterInfo >& v_element, int option) {
...
int packet_func::pacote071(packet& p, player *_session, std::multimap< int32_t , CaddieInfoEx >& v_element, int option) {
...
int packet_func::pacote073(packet& p, player *_session, std::multimap< int32_t , WarehouseItemEx >& v_element, int option) {
...
int packet_func::pacote0AA(packet& p, player *_session, std::vector< stItem >& v_item) {
	...
	if (v_item.size() > 0) {
		...
		p.addUint16((unsigned short)v_item.size());
...
int packet_func::pacote0D4(packet& p, player *_session, std::multimap< int32_t , CaddieInfoEx >& v_element) {
...
int packet_func::pacote0E1(packet& p, player *_session, std::multimap< int32_t , MascotInfoEx >& v_element, int option) {
...
int packet_func::pacote15C(packet& p, player *_session, uint32_t _uid, std::vector< MapStatistics >& v_ms, std::vector< MapStatistics >& v_msa, unsigned char season) {
	...
	p.addInt32((uint32_t)v_msa.size());
...
int packet_func::pacote1A9(packet& p, player *_session, int32_t ttl_milliseconds , int option) {
		...
		for (auto i = v_item.begin(); i != v_item.end(); ++i) {
			...
			if (i->type == 2) {
				try {
					...
					if (STDA_SOURCE_ERROR_DECODE(e.getCodeError()) == STDA_ERROR_TYPE::PACKET && STDA_ERROR_DECODE(e.getCodeError()) == 3 )
						p.addInt16(0);
					else
						throw;
				}
...
int packet_func::principal(packet& p, PlayerInfo *pi, ServerInfoEx& _si) {
	...
	p.addUint16(pi->mi.flag_login_time);
	p.addBuffer(&pi->mi.papel_shop, sizeof(pi->mi.papel_shop));
	p.addInt32(0);
	p.addUint64(  pi->block_flag.m_flag.ullFlag);
	p.addUint32(0);
	p.addInt32(_si.propriedade.ulProperty);
...
void packet_func::channel_broadcast(channel& _channel, packet& p, unsigned char _debug) {
	...
	std::vector< player* > channel_session = _channel.getSessions();
...
void packet_func::room_broadcast(room& _room, packet& p, unsigned char _debug) {
	...
	std::vector< player* > room_session = _room.getSessions(nullptr, false );
...
void packet_func::room_broadcast(room& _room, std::vector< packet* > v_p, unsigned char _debug) {
	...
	for (auto i = 0u; i < v_p.size(); ++i) {
		if (v_p[i] != nullptr) {
			room_session = _room.getSessions(nullptr, false );
...
int packet_func::packet1F4(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_SERVER(_arg1, _arg2);

	try {

		CHECK_SESSION_IS_AUTHORIZED("packet1F4(AntiCheat)");

		uint8_t tipo = pd._packet->readUint8();

		if (tipo == 1) {

			_smp::message_pool::getInstance().push(new message("[AntiCheat] Report recebido de " + std::string(pd._session.m_pi.nickname) + " (UID=" + std::to_string(pd._session.m_pi.uid) + ")", CL_FILE_LOG_AND_CONSOLE));
		}
		else if (tipo == 2) {
			uint8_t nonce_echo[16];
			pd._packet->readBuffer(nonce_echo, sizeof(nonce_echo));
			uint64_t client_tick = pd._packet->readUint64();
			uint8_t client_hmac[32];
			pd._packet->readBuffer(client_hmac, sizeof(client_hmac));

			std::string nick(pd._session.m_pi.nickname);
			std::string uid_s = std::to_string(pd._session.m_pi.uid);

			if (!pd._session.m_ac.hmac_key_sent) {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] CR ignorado (chave nao entregue) — " + nick + " (UID=" + uid_s + ")", CL_FILE_LOG_AND_CONSOLE));
				return 0;
			}

			if (!pd._session.m_ac.pending_nonce_valid) {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] CR sem nonce pendente — " + nick + " (UID=" + uid_s + ")", CL_FILE_LOG_AND_CONSOLE));
				return 0;
			}

			if (memcmp(nonce_echo, pd._session.m_ac.pending_nonce, sizeof(nonce_echo)) != 0) {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] CR FAIL (nonce divergente) — " + nick + " (UID=" + uid_s + ") — DISCONNECT", CL_FILE_LOG_AND_CONSOLE));
				pd._session.m_ac.pending_nonce_valid = false;
				gs->DisconnectSession(&pd._session);
				return 0;
			}

			uint8_t msg[16 + 8];
			memcpy(msg, nonce_echo, 16);
			for (int i = 0; i < 8; ++i)
				msg[16 + i] = (uint8_t)(client_tick >> (i * 8));

			uint8_t expected[32];
			hmac_sha256::compute(pd._session.m_ac.hmac_key, 32, msg, sizeof(msg), expected);

			bool ok = hmac_sha256::verify(expected, client_hmac);

			pd._session.m_ac.pending_nonce_valid = false;

			if (ok) {
				pd._session.m_ac.first_attest_received = true;
				_smp::message_pool::getInstance().push(new message("[AntiCheat] CR OK " + nick + " (UID=" + uid_s + ", tick=" + std::to_string(client_tick) + ")", CL_FILE_LOG_AND_CONSOLE));
			} else {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] CR FAIL (HMAC invalido) — " + nick + " (UID=" + uid_s + ") — DISCONNECT", CL_FILE_LOG_AND_CONSOLE));
				gs->DisconnectSession(&pd._session);
			}
		}
		else if (tipo == 3) {

			static const bool AC_KICK_ON_VIOLATION = false;

			uint8_t nonce_echo[16];
			pd._packet->readBuffer(nonce_echo, sizeof(nonce_echo));
			uint64_t client_tick = pd._packet->readUint64();
			uint32_t checks_bitmask = pd._packet->readUint32();
			uint8_t modules_hash[32];
			pd._packet->readBuffer(modules_hash, sizeof(modules_hash));
			uint8_t client_hmac[32];
			pd._packet->readBuffer(client_hmac, sizeof(client_hmac));
			...
			if (!pd._session.m_ac.hmac_key_sent || !pd._session.m_ac.pending_nonce_valid) {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] ATTEST ignorado (sem chave ou nonce) — " + nick + " (UID=" + uid_s + ")", CL_FILE_LOG_AND_CONSOLE));
				return 0;
			}
			if (memcmp(nonce_echo, pd._session.m_ac.pending_nonce, sizeof(nonce_echo)) != 0) {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] ATTEST FAIL (nonce divergente) — " + nick + " (UID=" + uid_s + ")" + (AC_KICK_ON_VIOLATION ? " — DISCONNECT" : " [SO LOG]"), CL_FILE_LOG_AND_CONSOLE));
				pd._session.m_ac.pending_nonce_valid = false;
				if (AC_KICK_ON_VIOLATION) gs->DisconnectSession(&pd._session);
				return 0;
			}

			uint8_t msg[16 + 8 + 4 + 32];
			memcpy(msg, nonce_echo, 16);
			for (int i = 0; i < 8; ++i) msg[16 + i] = (uint8_t)(client_tick >> (i * 8));
			for (int i = 0; i < 4; ++i) msg[24 + i] = (uint8_t)(checks_bitmask >> (i * 8));
			memcpy(msg + 28, modules_hash, 32);

			uint8_t expected[32];
			hmac_sha256::compute(pd._session.m_ac.hmac_key, 32, msg, sizeof(msg), expected);
			bool hmac_ok = hmac_sha256::verify(expected, client_hmac);
			pd._session.m_ac.pending_nonce_valid = false;

			if (!hmac_ok) {
				_smp::message_pool::getInstance().push(new message("[AntiCheat] ATTEST FAIL (HMAC invalido) — " + nick + " (UID=" + uid_s + ")" + (AC_KICK_ON_VIOLATION ? " — DISCONNECT" : " [SO LOG]"), CL_FILE_LOG_AND_CONSOLE));
				if (AC_KICK_ON_VIOLATION) gs->DisconnectSession(&pd._session);
				return 0;
			}

			auto to_hex = [](const uint8_t* b, size_t n) {
			...
			auto decode_bitmask = [](uint32_t bm) -> std::string {
				static const char* names[] = {
					"SP_A", "SP_B", "AIM_ALPHA", "AIM_DIAM", "AIM_RAIO",
					"AIM_DX", "AIM_DY", "AIM_SHOT", "DEBUGGER", "DBG_REMOTE",
					"HWBP", "CE", "HACK_TOOL", "HANDLE",
					"HOOK_TAMPERED", "SELF_TAMPERED", "MANUAL_MAPPED", "MONITOR_DEAD",
					"MODULE_FOREIGN"
				};
				std::string s;
				for (int i = 0; i < 19; ++i) {
			...
			static const uint8_t EXPECTED_MODULES_HASHES[][32] = {
				{ 0x21,0xe0,0xa7,0x9e,0x33,0x65,0x7c,0x53,0xd6,0x82,0x72,0x90,0x19,0xc2,0x88,0x42,
				  0x95,0x29,0x54,0x87,0x4f,0x62,0x57,0xaf,0x40,0x7e,0xee,0x28,0x7d,0xf8,0x45,0x34 },
				{ 0x26,0xc3,0xf9,0x5e,0xf2,0x7c,0x4b,0xb1,0xdf,0x5d,0xe7,0xa4,0x20,0x5e,0xc4,0x7e,
				  0x04,0xa5,0x3c,0x66,0xf6,0xa2,0x61,0xd9,0xd5,0x9a,0x38,0x80,0xd1,0xbb,0x72,0xc3 },
			};
			const int N_EXPECTED = (int)(sizeof(EXPECTED_MODULES_HASHES) / 32);
			bool expected_set = (N_EXPECTED > 0);

			bool modules_ok = false;
				for (int _k = 0; _k < N_EXPECTED; ++_k)
					if (memcmp(modules_hash, EXPECTED_MODULES_HASHES[_k], 32) == 0) { modules_ok = true; break; }
			bool checks_ok  = (checks_bitmask == 0);
			bool all_ok     = checks_ok && (modules_ok || !expected_set);

			if (all_ok) {
				pd._session.m_ac.first_attest_received = true;
				std::string log = "[AntiCheat] ATTEST OK " + nick + " (UID=" + uid_s + ")"
					+ (expected_set ? "" : " [baseline ausente]")
					+ " modhash=" + to_hex(modules_hash, 32);
				_smp::message_pool::getInstance().push(new message(log, CL_FILE_LOG_AND_CONSOLE));
			} else {

				char bmhex[16];
				snprintf(bmhex, sizeof(bmhex), "0x%08X", checks_bitmask);
				std::string log = "[AntiCheat] ATTEST FAIL " + nick + " (UID=" + uid_s + ")"
					+ " bitmask=" + bmhex
					+ " modhash=" + to_hex(modules_hash, 32);
				if (!checks_ok)    log += " [TAMPERED: " + decode_bitmask(checks_bitmask) + "]";
				if (!modules_ok && expected_set) log += " [MODULES MISMATCH]";
				log += AC_KICK_ON_VIOLATION ? " — DISCONNECT" : " [SO LOG]";
				_smp::message_pool::getInstance().push(new message(log, CL_FILE_LOG_AND_CONSOLE));
				if (AC_KICK_ON_VIOLATION) gs->DisconnectSession(&pd._session);
			}
		}
		else {
			_smp::message_pool::getInstance().push(new message("[AntiCheat] Tipo desconhecido (" + std::to_string((int)tipo) + ") de " + std::string(pd._session.m_pi.nickname), CL_FILE_LOG_AND_CONSOLE));
		}

	}catch (exception& e) {

		_smp::message_pool::getInstance().push(new message("[packet_func::packet1F4][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}

	return 0;
};

static void sendAntiCheatRaw(player* _session, uint16_t opcode, const uint8_t* payload, uint16_t payload_size) {
	if (_session == nullptr) return;

	std::vector<uint8_t> buf(8 + payload_size);
	buf[0] = 0xCA; buf[1] = 0xFE; buf[2] = 0xBA; buf[3] = 0xBE;
	buf[4] = (uint8_t)(opcode & 0xFF);
	buf[5] = (uint8_t)((opcode >> 8) & 0xFF);
	buf[6] = (uint8_t)(payload_size & 0xFF);
	buf[7] = (uint8_t)((payload_size >> 8) & 0xFF);
	if (payload_size > 0)
		memcpy(buf.data() + 8, payload, payload_size);

	_session->usa();
	try {
		_session->requestSendBuffer(buf.data(), buf.size(), true  );
	} catch (exception& e) {
		if (!STDA_ERROR_CHECK_SOURCE_AND_ERROR(e.getCodeError(), STDA_ERROR_TYPE::SESSION, 6))
			if (_session->devolve())
				sgs::gs::getInstance().DisconnectSession(_session);
		_smp::message_pool::getInstance().push(new message("[packet_func::sendAntiCheatRaw][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
		return;
	}
	if (_session->devolve())
		sgs::gs::getInstance().DisconnectSession(_session);
...
void packet_func::sendHmacKey(player* _session) {
	...
	try {
		...
		for (int i = 0; i < 32; ++i)
			_session->m_ac.hmac_key[i] = (uint8_t)(rd() & 0xFF);
		_session->m_ac.hmac_key_sent = true;
		_session->m_ac.key_sent_at = (uint64_t)GetTickCount64();

		sendAntiCheatRaw(_session, 0x01F6, _session->m_ac.hmac_key, 32);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendHmacKey][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};

void packet_func::sendChallenge(player* _session) {

	try {
		if (_session == nullptr) return;
		if (!_session->m_ac.hmac_key_sent) return;

		std::random_device rd;
		for (int i = 0; i < 16; ++i)
			_session->m_ac.pending_nonce[i] = (uint8_t)(rd() & 0xFF);

		_session->m_ac.nonce_sent_at = (uint64_t)GetTickCount64();
		_session->m_ac.last_challenge_at = _session->m_ac.nonce_sent_at;
		_session->m_ac.pending_nonce_valid = true;

		sendAntiCheatRaw(_session, 0x01F5, _session->m_ac.pending_nonce, 16);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendChallenge][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};

double packet_func::crazySeedPhase(uint32_t _seed, uint32_t _which) {
	uint32_t h = _seed * 1103515245u + 12345u + _which * 2654435761u;
	h ^= h >> 16;
	h = h * 2246822519u;
	h ^= h >> 13;
	return ((double)(h & 0xFFFFFF) / 16777216.0) * 6.283185307179586;
}

float packet_func::crazyExpectedFactor(uint32_t _seed, uint32_t _tickMs, float _rawMax, float _ampP) {

	const double wP = 8.125;

	if (_rawMax <= 1.0f)
		return 1.0f;

	double t = (double)_tickMs / 1000.0;
	double alvo = (double)_rawMax - (double)_ampP
	            + (double)_ampP * sin(t * wP + crazySeedPhase(_seed, 2));

	if (alvo < 10.0)
		alvo = 10.0;

	return (float)(alvo / (double)_rawMax);
}

void packet_func::sendCrazyClubSeed(player* _session, bool _on) {

	try {
		if (_session == nullptr) return;

		uint32_t seed = ((uint32_t)GetTickCount64() * 2654435761u)
		              ^ ((uint32_t)_session->m_pi.uid * 2246822519u)
		              ^ (uint32_t)rand();

		const float AMP_P = 5.0f;

		_session->m_ac.crazy_seed = seed;
		_session->m_ac.crazy_hole_started_at = GetTickCount64();
		_session->m_ac.crazy_active = _on;
		_session->m_ac.crazy_amp = AMP_P;

		uint32_t amp_bits;
		memcpy(&amp_bits, &AMP_P, 4);

		packet p((unsigned short)0x0BB8);
		p.addUint32(seed);
		p.addUint32(_on ? 1u : 0u);
		p.addUint32(amp_bits);

		packet_func::session_send(p, _session, 1);

		_smp::message_pool::getInstance().push(new message("[CRAZY][sendSeed][Log] player[UID="
			+ std::to_string(_session->m_pi.uid) + "] 0x0BB8 enviado seed=" + std::to_string(seed)
			+ " on=" + std::to_string(_on ? 1 : 0), CL_FILE_LOG_AND_CONSOLE));

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendCrazyClubSeed][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
}

void packet_func::sendBallRestoreFreeze(player* _session, float x, float y, float z) {
	...
	try {
		...
		sendAntiCheatRaw(_session, 0x01F7, payload, 12);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendBallRestoreFreeze][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};

void packet_func::sendShotCountRestore(player* _session, const unsigned char* _payload, unsigned short _size) {

	try {
		if (_session == nullptr || _payload == nullptr || _size == 0u) return;

		sendAntiCheatRaw(_session, 0x01F9, _payload, _size);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendShotCountRestore][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};

void packet_func::sendShotResultUnlock(player* _session) {

	try {
		if (_session == nullptr) return;

		sendAntiCheatRaw(_session, 0x01FA, nullptr, 0);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendShotResultUnlock][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};

void packet_func::sendApplyShotResult(player* _session) {

	try {
		if (_session == nullptr) return;

		sendAntiCheatRaw(_session, 0x01FB, nullptr, 0);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendApplyShotResult][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};

void packet_func::sendBallRestoreUnfreeze(player* _session) {

	try {
		if (_session == nullptr) return;

		sendAntiCheatRaw(_session, 0x01F8, nullptr, 0);

	} catch (exception& e) {
		_smp::message_pool::getInstance().push(new message("[packet_func::sendBallRestoreUnfreeze][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};
