...
void QuadTree3D::init_shot(Ball3D* _ball, Club3D* _club, Vector3D* _wind, ShotEndLocationData& _shot_cube, float _distance) {
	...
	m_shot = _shot_cube.special_shot;
	m_spike_init = -1;
	m_spike_med = -1;
	m_cobra_init = -1;
	...
	float pwr = _shot_cube.power_factor;

	m_power_range_shot = _shot_cube.power_club;

	m_power_factor = pwr;

	pwr *= std::sqrt(_shot_cube.porcentagem );
	...
	m_power_factor_shot = pwr;
	m_percent_shot_sqrt = std::sqrt(_shot_cube.porcentagem );

	m_ball->m_curve = _shot_cube.ball_point.x;
	m_ball->m_spin = _shot_cube.ball_point.y;

	pwr *= (std::abs(m_ball->m_curve) * 0.1f) + 1.f;

	m_ball->m_velocity = Vector3D(_shot_cube.ball_velocity.x, _shot_cube.ball_velocity.y, _shot_cube.ball_velocity.z);

	m_ball->m_rotation_curve = _shot_cube.ball_rotation_curve;
	m_ball->m_rotation_spin = _shot_cube.ball_rotation_spin;
