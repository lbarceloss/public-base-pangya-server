...
void MgrDailyQuest::requestTakeRewardQuest(player& _session, packet *_packet) {
	...
	try {
		...
		auto v_quest = leaveQuestUser(_session, quest_id, num_quest);
...
bool MgrDailyQuest::checkCurrentQuestUser(DailyQuestInfo& _dqi, player& _session) {
	...
	if (pi.dqiu.current_date != 0) {
		...
		translateDateLocal(pi.dqiu.current_date, &st2);
...
std::vector< RemoveDailyQuestUser > MgrDailyQuest::getOldQuestUser(player& _session) {
	...
	std::for_each(map_ai.begin(), map_ai.end(), [&](auto& el) { if (el.second.status == AchievementInfo::PENDENTING ) v_old_quest.push_back({ el.second.id, el.second._typeid }); });

	if (v_old_quest.empty()) {
...
int32_t MgrDailyQuest::addCounterItemUser(player& _session, CounterItemInfo& _cii) {
	...
	CmdAddCounterItem cmd_aci(_session.m_pi.uid, _cii._typeid, _cii.value , true);
...
void MgrDailyQuest::updateDailyQuest(DailyQuestInfo& _dqi) {
	...
	auto map_qi = sIff::getInstance().getQuestItem();

	std::vector< uint32_t > a, b, c;

	std::for_each(map_qi.begin(), map_qi.end(), [&](auto& el) {
		if (((el.second._typeid & 0x00FFFFFF) >> 16) < 0x40) {
	...
	CmdUpdateDailyQuest cmd_udq(_dqi, true);
