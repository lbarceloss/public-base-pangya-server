...
void MgrAchievement::initAchievement(bool _create) {
	...
	if (!_create) {
		...
		CmdCheckAchievement cmd_cAchieve(m_uid, true);
		...
		if (!cmd_cAchieve.getLastState()) {
			...
			auto qi = sIff::getInstance().findQuestItem(CLEAR_10_DAILY_QUEST_TYPEID);
			...
			CmdAchievementInfo cmd_ai(m_uid, true);
...
AchievementInfoEx MgrAchievement::createAchievement(uint32_t _uid, IFF::Achievement& _achievement, AchievementInfo::ACHIEVEMENT_STATUS _status) {
    ...
    CmdCreateQuest cmd_cq(_uid, true);
    CmdCreateAchievement cmd_ca(_uid, true);
