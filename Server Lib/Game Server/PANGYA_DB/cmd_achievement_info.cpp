...
void CmdAchievementInfo::lineResult(result_set::ctx_res* _result, uint32_t  ) {
...
inline void CmdAchievementInfo::check_quest_achievement(AchievementInfoEx& _ai, QuestStuffInfo& _qsi) {
	...
	if (_ai.status == 3  && (_ai.quest_base_typeid == 0 || _qsi._typeid == _ai.quest_base_typeid) && _qsi.counter_item_id <= 0) \
