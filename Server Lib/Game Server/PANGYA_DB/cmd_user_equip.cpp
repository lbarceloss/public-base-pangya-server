...
void CmdUserEquip::lineResult(result_set::ctx_res* _result, uint32_t  ) {
	...
	for (i = 0; i < 10; i++)
		m_ue.item_slot[i] = IFNULL(atoi, _result->data[4 + i]);
	for (i = 0; i < 6; i++)
		m_ue.skin_id[i] = IFNULL(atoi, _result->data[14 + i]);
	for (i = 0; i < 6; i++)
		m_ue.skin_typeid[i] = IFNULL(atoi, _result->data[20 + i]);
	m_ue.mascot_id = IFNULL(atoi, _result->data[26]);
	for (i = 0; i < 2; i++)
		m_ue.poster[i] = IFNULL(atoi, _result->data[27 + i]);
}
