...
void CmdUserInfo::lineResult(result_set::ctx_res* _result, uint32_t  ) {
	...
	for (i = 0; i < 5; i++)
		m_ui.best_score[i] = (unsigned char)IFNULL(atoi, _result->data[24 + i]);
	for (i = 0; i < 5; i++)
		m_ui.best_pang[i] = IFNULL(atoll, _result->data[29 + i]);
