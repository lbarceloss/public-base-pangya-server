...
void CmdFindWarehouseItem::lineResult(result_set::ctx_res* _result, uint32_t  ) {
	...
	if (m_wi.id > 0) {
		...
		for (i = 0; i < 5; i++)
			m_wi.c[i] = (short)IFNULL(atoi, _result->data[4 + i]);
		...
		for (i = 0; i < 4; i++)
			m_wi.card.character[i] = IFNULL(atoi, _result->data[15 + i]);
		for (i = 0; i < 4; i++)
			m_wi.card.caddie[i] = IFNULL(atoi, _result->data[19 + i]);
		for (i = 0; i < 4; i++)
			m_wi.card.NPC[i] = IFNULL(atoi, _result->data[23 + i]);
		m_wi.clubset_workshop.flag = (short)IFNULL(atoi, _result->data[27]);
		for (i = 0; i < 5; i++)
			m_wi.clubset_workshop.c[i] = (short)IFNULL(atoi, _result->data[28 + i]);
