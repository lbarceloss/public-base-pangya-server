...
void CmdCoinCubeLocationInfo::lineResult(result_set::ctx_res* _result, uint32_t  ) {
	...
	CubeEx cube(
		(uint32_t)IFNULL(atoi, _result->data[0]),
		Cube::eTYPE((uint32_t)IFNULL(atoi, _result->data[3])),
		0u,
		Cube::eFLAG_LOCATION((uint32_t)IFNULL(atoi, _result->data[4])),
		(float)IFNULL(atof, _result->data[6]),
		(float)IFNULL(atof, _result->data[7]),
		(float)IFNULL(atof, _result->data[8]),
