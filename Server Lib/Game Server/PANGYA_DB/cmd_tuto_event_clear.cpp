...
void CmdTutoEventClear::lineResult(result_set::ctx_res*  , uint32_t  ) {
...
response* CmdTutoEventClear::prepareConsulta(database& _db) {
	...
	auto r = _update(_db, m_szConsulta[0] + std::string(m_type == T_ROOKIE ? m_szConsulta[1] : (m_type == T_BEGINNER ? m_szConsulta[2] : m_szConsulta[3] )) + m_szConsulta[4] + std::to_string(m_uid));
