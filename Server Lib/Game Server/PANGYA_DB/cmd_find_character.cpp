...
void CmdFindCharacter::lineResult(result_set::ctx_res* _result, uint32_t  ) {
	...
	if (m_ci.id > 0) {
		...
		for (i = 0; i < 24; i++)
			m_ci.parts_id[i] = IFNULL(atoi, _result->data[2 + i]);
		for (i = 0; i < 24; i++)
			m_ci.parts_typeid[i] = IFNULL(atoi, _result->data[26 + i]);
		...
		for (i = 0; i < 5; i++)
			m_ci.pcl[i] = (unsigned char)IFNULL(atoi, _result->data[53 + i]);
		m_ci.purchase = (unsigned char)IFNULL(atoi, _result->data[58]);
		for (i = 0; i < 5; i++)
			m_ci.auxparts[i] = IFNULL(atoi, _result->data[59 + i]);
		for (i = 0; i < 4; i++)
			m_ci.cut_in[i] = IFNULL(atoi, _result->data[64 + i]);
		m_ci.mastery = IFNULL(atoi, _result->data[68]);
		for (i = 0; i < 4; i++)
			m_ci.card_character[i] = IFNULL(atoi, _result->data[69 + i]);
		for (i = 0; i < 4; i++)
			m_ci.card_caddie[i] = IFNULL(atoi, _result->data[73 + i]);
		for (i = 0; i < 4; i++)
			m_ci.card_NPC[i] = IFNULL(atoi, _result->data[77 + i]);
	}
}
