...
void CmdCaddieInfo::lineResult(result_set::ctx_res* _result, uint32_t  ) {
	...
	if (it == v_ci.end() || (v_ci.count(ci.id) == 1 && it->second._typeid != ci._typeid))
...
std::multimap< int32_t , CaddieInfoEx >& CmdCaddieInfo::getInfo() {
	return v_ci;
}
