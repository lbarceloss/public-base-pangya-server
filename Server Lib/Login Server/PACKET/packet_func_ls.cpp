...
int packet_func::packet003(void* _arg1, void* _arg2) {
	...
	try {
		...
		CmdRegisterLogonServer cmd_rls(pd._session.m_pi.uid, server_uid, true );
		...
		CmdAuthKeyGame cmd_auth_key_game(pd._session.m_pi.uid, server_uid, true );
		...
		if (!STDA_ERROR_CHECK_SOURCE_AND_ERROR(e.getCodeError(), STDA_ERROR_TYPE::EXEC_QUERY, 6 ))
			throw;
	}
...
int packet_func::packet006(void* _arg1, void* _arg2) {
	...
	try {
		...
		CmdSaveNick cmd_sn(pd._session.m_pi.uid, wnick, true );
		...
		CmdAddFirstLogin cmd_afl(pd._session.m_pi.uid, 1, true );
		...
		CmdFirstSetCheck cmd_fsc(pd._session.m_pi.uid, true );
		...
		pacote00E(p, &pd._session, wnick, 1 );
		session_send(p, &pd._session, 1);
...
int packet_func::packet007(void* _arg1, void* _arg2) {
	...
	try {
		...
		if (nc == SUCCESS) {
			CmdVerifNick cmd_vn(wnick, true );
...
int packet_func::packet008(void* _arg1, void* _arg2) {
	...
	try {
		...
		CmdAddCharacter cmd_ac(pd._session.m_pi.uid, ci, 0, 1, true );
		CmdAddFirstSet cmd_afs(pd._session.m_pi.uid, true );
		...
		CmdUpdateCharacterEquiped cmd_uce(pd._session.m_pi.uid, ci.id, true );
...
int packet_func::pacote001(packet& p, player *_session, int option) {
	...
	if (option == 0) {
		...
		p.addInt32(0);
		p.addInt32(5);
		p.addFixedString(formatDateLocal(0), 19);
		p.addString("302540");
		p.addUint32(0);
		p.addUint32(0);
...
inline void packet_func::succes_login(void* _arg, player *_session, int option) {
	...
	try {
		...
		CmdServerList cmd_server_list(CmdServerList::GAME, true );
		...
		CmdAuthKeyLogin cmd_auth_key_login(_session->m_pi.uid, true );
		...
		if (option == 0) {
			CmdChatMacroUser cmd_macro_user(_session->m_pi.uid, true );
		...
		CmdRegisterPlayerLogin cmd_rpl(_session->m_pi.uid, ip, ls->getUID(), true );
		...
		if (STDA_SOURCE_ERROR_DECODE(e.getCodeError()) == STDA_ERROR_TYPE::EXEC_QUERY) {
			if (STDA_ERROR_DECODE(e.getCodeError()) != 7  && STDA_ERROR_DECODE(e.getCodeError()) != 9
				&& STDA_ERROR_DECODE(e.getCodeError()) != 8  && STDA_ERROR_DECODE(e.getCodeError()) != 5 )
				throw;
		}else
			throw;
	}
