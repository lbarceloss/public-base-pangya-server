...
gg_auth_server::gg_auth_server() : unit(m_player_manager, 2 , 2 )
	...
	try {
		...
		m_game_guard = new GGAuth(m_si.max_user * 2000 );
...
void gg_auth_server::requestLoginSecondStep(player& _session, packet *_packet) {
	...
	try {
		...
		try {
			...
			check_key = md5::hash((const unsigned char*)check_key.data(), (unsigned long)check_key.length());
...
void gg_auth_server::onAcceptCompleted(session* _session) {
	...
	packet _packet((unsigned short)0x00);

	_packet.addInt32((long)_session->m_key);
	...
	try {
		...
		_session->requestSendBuffer(mb.buf, mb.len, true );
	}catch (exception& e) {
...
void gg_auth_server::authCmdBroadcastNotice(std::string  ) {

	return;
}

void gg_auth_server::authCmdBroadcastTicker(std::string  , std::string  ) {

	return;
}

void gg_auth_server::authCmdBroadcastCubeWinRare(std::string  , uint32_t  ) {
...
void gg_auth_server::authCmdConfirmDisconnectPlayer(uint32_t  ) {

	return;
}

void gg_auth_server::authCmdNewMailArrivedMailBox(uint32_t  , uint32_t  ) {

	return;
}

void gg_auth_server::authCmdNewRate(uint32_t  , uint32_t  ) {

	return;
}

void gg_auth_server::authCmdReloadGlobalSystem(uint32_t  ) {
