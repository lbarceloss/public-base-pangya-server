...
void SmartCalculatorPlayer::macro(const char* _args) {
	...
	try {
		...
		if (!m_special_slope_flag)
			...
			float slope = (float)( -((cosf(rad) * m_special_slope_break[0]) - (-sinf(rad) * m_special_slope_break[2])));
...
void SmartCalculatorPlayer::make_slope_break(const char* _args) {
	...
	try {
		...
		if (v_args.size() == 2) {
			...
			m_special_slope_break[0] = ((float)std::atof(v_args[0].c_str())) / scala * -1 ;
			m_special_slope_break[2] = ((float)std::atof(v_args[1].c_str())) / scala * -1 ;
			...
			m_special_slope_break[2] = ((float)std::atof(v_args[2].c_str())) / scala * -1 ;
