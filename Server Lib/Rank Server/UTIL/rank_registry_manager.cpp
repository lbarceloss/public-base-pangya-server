	...
	try {
		...
		if (it != m_entry.end()) {
			...
			if (range.second && range.first.first != range.first.second && std::distance(range.first.first, range.first.second) > 0u) {
				...
				_packet.addUint32(_sd.page);
				_packet.addUint32(NUMBER_OF_PAGE_MENU_REGISTRY((uint32_t)it->second.size()));
				...
				for (auto it_entry = range.first.first; it_entry != range.first.second; ++it_entry) {
					...
					if ((it_chr_entry = m_character_entry.find(it_entry->second.getUID())) != m_character_entry.end())
						...
						_packet.addZeroByte(7u);
					}
				}

			}else
				_packet.addZeroByte(10);
	...
	try {
		...
		if (it_entry != m_entry.end()) {
			...
			if (it != it_entry->second.end()) {
				...
				_packet.addUint8(ePLAYER_POSITION_RANK_TYPE::PPRT_NOT_RANK);
...
void RankRegistryManager::sendPlayerFullInfo(player& _session, uint32_t _uid ) {
	...
	try {
		...
		if (it != m_entry.end()) {
			...
			if (range.second && range.first.first != range.first.second && std::distance(range.first.first, range.first.second) > 0u) {
				...
				p.addUint32(_sd.page);
				p.addUint32(NUMBER_OF_PAGE_MENU_REGISTRY((uint32_t)it->second.size()));
...
FoundPlayer RankRegistryManager::searchPlayerByNickname(std::string _nickname, search_dados& _sd) {
	...
	try {
		...
		ret.first = (it_entry->second.getCurrentPosition() - 1u) % LIMIT_REGISTRY_FOR_PAGE;
...
void RankRegistryManager::initialize() {
	...
	try {
		...
		CmdRankRegistryInfo cmd_rri(true);
		...
		CmdRankRegistryCharacterInfo cmd_rrci(true);
...
#if defined(_WIN32)
					_mkdir(tmp_dir.c_str()) == 0
#elif defined(__linux__)
					mkdir(tmp_dir.c_str(), MODE_DIR_LOG) == 0
#endif
				)
					dir = tmp_dir;
				else
