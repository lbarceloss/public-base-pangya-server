...
\
													...
													if (left > (int)dwIOsize) { \
														...
														dwIOsize -= left; \
\
														_packet = new packet((size_t)_session->_get_func()->getSizeMaked() ); \
...
\
														if (((_ph).size + (_sz_ph)) <= dwIOsize) { \
															_packet = new packet((size_t)((_ph).size + (_sz_ph)) ); \
...
\
															...
															_session->_set_func(new packet((size_t)((_ph).size + (_sz_ph)) )); \
															_session->_get_func()->add_maked(lpBuffer->getBuffer(), dwIOsize); \
															lpBuffer->consume(dwIOsize); \
															dwIOsize = 0; \
														} \
													}else { \
														_session->_set_func(new packet((size_t)0 )); \
														_session->_get_func()->add_maked(lpBuffer->getBuffer(), dwIOsize); \
														lpBuffer->consume(dwIOsize); \
														dwIOsize = 0; \
													} \
...
unit_connect_base::unit_connect_base(ServerInfoEx& _si)
	...
	try {
		...
		m_threads.push_back(new thread(TT_WORKER_SEND, unit_connect_base::_worker_send, (LPVOID)this, THREAD_PRIORITY_ABOVE_NORMAL));
		...
		m_thread_monitor = new thread(TT_MONITOR, unit_connect_base::_monitor, (LPVOID)this );
...
void unit_connect_base::postIoOperation(session* _session, Buffer* lpBuffer, DWORD dwIOsize, DWORD operation) {
	...
	m_iocp_send.postStatus((ULONG_PTR)_session, 0, (LPOVERLAPPED)tmp);
#elif defined(__linux__)
			...
			try {
				...
				if ((ep_ev.events & EPOLLERR) == EPOLLERR) {
					...
					if (err_socket == ETIMEDOUT) {
						...
						translate_operation(_session, 0 , lpBuffer, STDA_OT_RECV_COMPLETED);
	...
	try {
		...
		while (1) {
			try {
				...
				m_iocp_logical.getStatus((PULONG_PTR)&_session, &dwIOsize, (LPOVERLAPPED*)&lpMyOver);
	...
	try {
		...
		while (1) {
			try {
				...
				m_iocp_send.getStatus((PULONG_PTR)&_session, &dwIOsize, (LPOVERLAPPED*)&lpMyOver);
...
#if defined(_WIN32)
		} while (WaitForSingleObject(hEventContinueMonitor, 5000 ) == WAIT_TIMEOUT);

		auto last_error = GetLastError();
#elif defined(__linux__)
		} while (hEventContinueMonitor != nullptr && hEventContinueMonitor->wait(5000 ) == WAIT_TIMEOUT);
