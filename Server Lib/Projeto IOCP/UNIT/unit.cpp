...
unit::unit(session_manager& _session_manager, uint32_t _db_instance_num, uint32_t _job_thread_num)
	...
	try {
		...
		m_thread_monitor = new thread(TT_MONITOR, unit::_monitor, (LPVOID)this );
		...
		m_threads.push_back(new thread(TT_ACCEPT, unit::_accept, (LPVOID)this, THREAD_PRIORITY_ABOVE_NORMAL ));
	...
	try {
		...
		while (InterlockedCompareExchange(&m_continue_accpet, 1, 1)) {
			try {
				...
				if (STDA_SOURCE_ERROR_DECODE(e.getCodeError()) != STDA_ERROR_TYPE::PLAYER_MANAGER || STDA_ERROR_DECODE(e.getCodeError()) != 3 )
					throw;
				else {
...
void unit::accept_completed(SOCKET*  , DWORD  , myOver*  , DWORD  ) {
};
