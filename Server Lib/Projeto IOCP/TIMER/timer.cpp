...
DWORD timer::getElapsed() {
	...
	return (DWORD)((m_elapsed.QuadPart - m_start.QuadPart) * 1000  / m_frequency.QuadPart);
#elif defined(__linux__)
...
void timer::init() {
	...
	if (!CreateTimerQueueTimer(&m_hTimer, m_hTimerQueue,
			...
			0 ,
			(m_tipo == PERIODIC) ? periodic : m_time ,
...
void timer::_stop(TIMER_STATE _state) {
	...
	try {
		...
		if (getState() != _state + 1 && getState() != _state + 2) {
			...
			} while (error == 0  && lastError != ERROR_IO_PENDING && count++ < LIMIT_COUNT_TRY);
			...
			} while (error <= 0  && error != ERROR_IO_PENDING && count++ < LIMIT_COUNT_TRY);
...
void timer::check_event_end() {
	...
	if (m_hEvent != nullptr && m_hEvent->wait(INFINITE) != WAIT_OBJECT_0)
		throw exception("[timer::check_event_end][Error] Ao esperar evento.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::TIMER, 8, errno));
#endif
...
void timer::hasTimerTick() {
	...
	if (getState() == STANDBY) {
		...
		auto d = (DWORD)((m_elapsed.QuadPart - m_start.QuadPart) * 1000  / m_frequency.QuadPart);
#elif defined(__linux__)
		clock_gettime(CLOCK_TICK_TO_TIMER, &m_elapsed);
