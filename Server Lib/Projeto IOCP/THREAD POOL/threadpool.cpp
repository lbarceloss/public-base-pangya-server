...
threadpool::threadpool(size_t _num_thread_workers_io, size_t _num_thread_workers_logical, uint32_t _job_thread_num)
	...
	m_thread_console = new thread(TT_CONSOLE, threadpool::_console, (LPVOID)this, THREAD_PRIORITY_NORMAL );
	...
	for (i = 0; i < _num_thread_workers_logical; i++) {
		m_threads.push_back(new thread(TT_WORKER_SEND, threadpool::_worker_send, (LPVOID)new threadpool_and_index{ this, (uint32_t)i }, THREAD_PRIORITY_ABOVE_NORMAL));
	...
	try {
		...
		while (1) {
			try {
				...
				translate_operation((session*)&listener, 1 , (Buffer*)lpBuffer, STDA_OT_ACCEPT_COMPLETED);
...
void threadpool::translate_operation(session *_session, DWORD dwIOsize, Buffer *lpBuffer, DWORD _operation) {
	...
	switch (_operation) {
		...
		default:
		{
			...
			translate_packet(_session, lpBuffer, 0 , STDA_OT_RECV_COMPLETED);
