...
response* mssql::ExecQuery(std::wstring _query) {
	...
	try {
		...
		do {
			...
			HandleDiagnosticRecord(m_ctx.hStmt, SQL_HANDLE_STMT, GERAL_ERROR, SQLRowCount(m_ctx.hStmt, &numRows), _query);
		...
		} while ((ret = SQLMoreResults(m_ctx.hStmt)) == SQL_SUCCESS);
...
#elif defined(__linux__)
		if (std::u16string((const char16_t*)wszState).compare(u"01004") ) {
