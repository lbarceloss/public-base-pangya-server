...
std::string md5::hash(const unsigned char* _data, uint32_t _size) {
    ...
    if (!CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT))
        throw exception("[md5::hash][Error] nao conseguiu adiquirir o Crypt Context.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 1, GetLastError()));

    if (!CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) {
        DWORD err = GetLastError();
        CryptReleaseContext(hProv, 0);
        throw exception("[md5::hash][Error] nao conseguiu Criar o Crypt Hash MD5.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 2, err));
    }

    if (!CryptHashData(hHash, _data, _size, 0)) {
        DWORD err = GetLastError();
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, 0);
        throw exception("[md5::hash][Error] nao conseguiu processar os dados para o hash do Crypt MD5.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 3, err));
    }

    unsigned char hashv[MD5LEN];
    DWORD cbHash = MD5LEN;

    if (!CryptGetHashParam(hHash, HP_HASHVAL, hashv, &cbHash, 0)) {
        DWORD err = GetLastError();
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, 0);
        throw exception("[md5::hash][Error] nao conseguiu pegar o Hash Param do Crypt Hash MD5.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 5, err));
    }
    ...
    if (!MD5_Init(&ctx))
        throw exception("[md5::hash][Error] nao conseguiu Inicializar o MD5 Context.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 2, errno));

    if (!MD5_Update(&ctx, _data, _size))
        throw exception("[md5::hash][Error] nao conseguiu processar os dados para o hash do MD5.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 3, errno));

    unsigned char hashv[MD5_DIGEST_LENGTH];
    DWORD cbHash = MD5_DIGEST_LENGTH;

    if (!MD5_Final(hashv, &ctx))
        throw exception("[md5::hash][Error] nao conseguiu finalizar o hash do MD5.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::MD5, 5, errno));
#endif

    for (auto i = 0u; i < cbHash; ++i)
        sHash += hex_util::ltoaToHex((uint32_t)hashv[i]);
