...
inline SYSTEMTIME stdA::TzLocalUnixToSystemTime(time_t _time_unix) {
	...
	if (TzSpecificLocalTimeToSystemTime(NULL , &st, &tzUTC) == 0)
		error = true;
...
inline SYSTEMTIME stdA::TzLocalTimeToSystemTime(SYSTEMTIME& _st) {
	...
	if (TzSpecificLocalTimeToSystemTime(NULL , &_st, &tzUTC) == 0)
		error = true;
...
SYSTEMTIME stdA::UnixUTCToTzLocalTime(time_t _time_unix) {
	...
	if (SystemTimeToTzSpecificLocalTime(NULL , &utc, &tzLocal) == 0)
		error = true;
...
inline SYSTEMTIME stdA::SystemTimeToTzLocalTime(SYSTEMTIME& _st) {
	...
	if (SystemTimeToTzSpecificLocalTime(NULL , &_st, &tzLocal) == 0)
		error = true;
...
int64_t stdA::getHourDiff(SYSTEMTIME& _st1, SYSTEMTIME& _st2) {
	return ((((((((_st1.wDay - _st2.wDay) * 24 ) + (_st1.wHour - _st2.wHour)) * 60 ) + (_st1.wMinute - _st2.wMinute)) * 60 ) + (_st1.wSecond - _st2.wSecond)) * 1000 ) + (_st1.wMilliseconds - _st2.wMilliseconds);
};
