...
Buffer::Buffer( void* buffer, size_t size, int seq_mode) :  m_seq_mode(seq_mode) {
...
void Buffer::init( void* buffer, size_t size, int seq_mode) {
...
void Buffer::consume(size_t size) {
	...
	}else if ((m_index_w - size) > MAX_BUFFER_SIZE ) {
#if defined(_WIN32)
		LeaveCriticalSection(&m_cs_wr);
#elif defined(__linux__)
		pthread_mutex_unlock(&m_cs_wr);
