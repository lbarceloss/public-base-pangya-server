			...
			for (index = 0; index < num_entries; ++index) { \
				...
				if (strcmp(st.name, (iff_name)) == 0) { \
					...
					if (file != nullptr && ZIP_GET_REAL_SIZE(st.size) >= sizeof(IFF::Head)) { \
						...
						\
						if (head.version == IFF_VERSION  && (head.count_element * sizeof(type) + sizeof(head)) == ZIP_GET_REAL_SIZE(st.size)) { \
							type _iff; \
							\
...
uint32_t iff::getItemSubGroupIdentify24(uint32_t _typeid) {
	return (uint32_t)((_typeid & ~0xFC000000) >> 24);
};

uint32_t iff::getItemSubGroupIdentify22(uint32_t _typeid) {
	return (uint32_t)((_typeid & ~0xFC000000) >> 22);
};

uint32_t iff::getItemSubGroupIdentify21(uint32_t _typeid) {
	return (uint32_t)((_typeid & ~0xFC000000) >> 21);
};
...
uint32_t iff::getCaddieIdentify(uint32_t _typeid) {
	return (uint32_t)(((_typeid & 0x0FFF0000) >> 21)  + ((_typeid & 0x000F0000) >> 16) );
};
...
bool iff::IsCanOverlapped(uint32_t _typeid) {
	...
	case AUX_PART:
	{
		...
		if (auxPart != nullptr && auxPart->cc[0]  > 0)
...
bool iff::IsItemEquipable(uint32_t _typeid) {
	...
	if (item != nullptr)
		return (getItemSubGroupIdentify24(_typeid) >> 1) == 0;
