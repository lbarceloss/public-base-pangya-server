...
packet::packet( unsigned short _tipo) :   m_tipo(_tipo) {
    m_plain.clear();
    m_maked.clear();
...
void packet::init_plain( unsigned short _tipo) {
    m_tipo = _tipo;
...
void packet::make(unsigned char _key) {
    ...
    phc.size = (unsigned short)m_plain.m_index_w + 1 ;
...
void packet::makeFull(unsigned char _key) {
    ...
    ph.size = (unsigned short)compress_out + 5 ;
