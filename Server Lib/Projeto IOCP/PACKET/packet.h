...
#pragma pack()
    ...
    public:
        inline uint32_t getNumberNS() {
            return unionConvertido.dwConvertido;
        };

        inline uint32_t getNumberIS() {
            uint32_t ulNumber = 0;
    ...
    class packet {
        public:
            packet();
			explicit packet(size_t _size );

            explicit packet( unsigned short _tipo);
            ~packet();

            void init_maked();
			void init_maked(size_t _size);
            void init_plain( unsigned short _tipo);
