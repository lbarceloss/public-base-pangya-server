...
#define MAKE_END_SPLIT_PACKET(_debug) packet_func::session_send(p, _session, (_debug));  \
		} \

#define MAKE_END_SPLIT_PACKET_REF(_debug) packet_func::session_send(p, &_session, (_debug));  \
		} \
...
namespace stdA {
    ...
    class packet_func_base {
		...
		public:
			static func_arr funcs;
            static func_arr funcs_sv;
			static func_arr funcs_as;
    };
}
