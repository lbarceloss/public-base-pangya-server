...
#endif
{
	...
	for (i = 0; i < 0x10000u; i++) {
		...
		while (hist) {
			...
			if ((seconds = getConnectTime(hist->sock)) < 0 || seconds < (DIFF_TICK(tick, hist->tick, frequency) / 1000 ) ) {
				prev_hist->next = hist->next;
				free(hist);
				hist = prev_hist->next;
				clear++;
...
socket::socket(int option) {
	...
	ddos_count = 10;
	ddos_interval = 3 * 1000;
	ddos_autoreset = 10 * 60 * 1000;

	limit_connection_per_ip = 3;
...
#if defined(_WIN32)
		InterlockedExchange(&m_thread_check_on, 1);
