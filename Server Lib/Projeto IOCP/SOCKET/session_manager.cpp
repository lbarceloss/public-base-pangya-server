...
session* session_manager::getSessionToDelete(DWORD _dwMilliseconds) {
	...
	try {
		...
		if (!STDA_ERROR_CHECK_SOURCE_AND_ERROR(e.getCodeError(), STDA_ERROR_TYPE::LIST_ASYNC, 2 ))
			throw;
...
void session_manager::checkSessionLive() {
	...
	for (auto& el : m_sessions) {
		...
		if (el->m_sock.fd != INVALID_SOCKET) {
			...
			}else if (m_TTL > 0 && ((std::clock() - el->m_tick) / (double)CLOCKS_PER_SEC) > (double)(m_TTL / 1000.0)) {
